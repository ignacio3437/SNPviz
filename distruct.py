...
def plot_admixture(admixture, population_indices, population_labels, title):
    ...
    N,K = admixture.shape
    colors = plot.get_cmap('tab10')(range(K))
    plot.get_cmap('tab10')(np.arange(N))
    text_color = 'k'
    bg_color = 'w'
    fontsize = 12
    ...
    for p,popname in enumerate(population_labels):
        ...
        if indices.size>0:
            vline_pos = (indices.max()+.5)*indiv_width
            subplot.axvline(vline_pos, linestyle='-', linewidth=0.6, c='#ffffff')
...
def get_admixture_proportions(params):
    ...
    try:
        # handle = open(f'{params['inputfile']}.{params['K']}.meanQ','r')
        handle = open('%s.%s.meanQ'%(params['inputfile'],params['K']),'r')
    except:
        handle = open('%s.%s.Q'%(params['inputfile'],params['K']),'r')
...
def distructor(params):
    ...
    figure.savefig(params['output'], dpi=300)
    return
...
if __name__=="__main__":
    ...
    parameters = [params['K'], params['input'], params['output'], params['popfile'], params['title']]
    distructor(params)
